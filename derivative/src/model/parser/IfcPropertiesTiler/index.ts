...
export class IfcPropertiesTiler {
  ...
  constructor(
    ...
    private onIndicesStreamed: (payload: string) => void,
    private onPropertiesStreamed: (payload: {
      type: number;
      data: {[id: number]: any};
    }) => void,
  ...
  async streamFromBuffer() {
    try {
      ...
      console.log(
        `Streaming property took ${(performance.now() - before) / 1000} ms!`
      );
    } catch (error: any) {
  ...
  private async streamAllProperties() {
    ...
    this.onIndicesStreamed(this.relations.serializeRelations(rels));
