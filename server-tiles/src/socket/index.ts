...
export type ICredentialType = "project" | "room";

export interface IRoomCredential {
  member: IRoomMember;
  roomId: string;
}
...
export class SocketService {
  ...
  private onConnection = (socket: Socket) => {
    const {credential, type} = socket.handshake.auth;
    if (!credential || typeof type !== "string") return;
    const connectType = type as ICredentialType;
    if (connectType === "project") {
      this.onProjectConnection(socket, credential);
    } else if (connectType === "room") {
      this.onRoomConnection(socket, credential);
    }
  };

  private onProjectConnection = (socket: Socket, userId: string) => {
    socket.join(userId);
   ...
   * @returns
   */
  onNotify = (userId: string, project: any) => {
    if (!this.io) return;
    this.io.to(userId).emit("update-model", project);
  };
  /**
   *
   * @param socket
   * @param credential
   */
  private onRoomConnection = (socket: Socket, credential: IRoomCredential) => {
    const {member, roomId} = credential;
...
let webSocket: SocketService | null = null;

export function onConnectSocket(server: any) {
  if (!webSocket) {
    webSocket = new SocketService(server);
  }
}

export function onNotify(userId: string, project: any) {
  if (!webSocket) return;
  webSocket.onNotify(userId, project);
}
